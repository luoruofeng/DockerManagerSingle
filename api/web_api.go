...
func opContainerWithCmds(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	cid := vars["container_id"]
	cmds := vars["cmds"]

	oh := newOpContainerHandler(cid)
	wsh := NewWebSocketHandle(w, r, oh)
	go wsh.Run([]string{cmds})
}

func opContainer(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	cid := vars["container_id"]
...
func addHandler(r *mux.Router) {
	...
	r.HandleFunc("/container/op/{container_id}", opContainer).Methods("GET", "PORT")
	r.HandleFunc("/container/op/{container_id}/{cmds}", opContainerWithCmds).Methods("GET", "PORT")
